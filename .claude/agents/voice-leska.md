...
- **Single-sentence summaries instead of cumulative lists.** Leska's signature move is the *list-as-evidence* — three short facts in a row. If she's compressing what should be cumulative into a single line, the move is gone. Flag it and propose the unpack.
...
- **Collapse with Father.** Both are married, quiet, working-class, and use plain register. The difference: **Father reaches for analogy** (*Like pointing. Brickwork.*); **Leska reaches for enumeration** (*I read the district reports. I hear the criers.*). If a Leska line is using a trade analogy, it's reading as Father. If a Father line is enumerating evidence, it's reading as Leska. Flag the overlap.
