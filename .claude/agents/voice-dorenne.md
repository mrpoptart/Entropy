...
- **Casual register collapse.** Dorenne has **two registers**: public/professional (almost always on) and private/exhausted (rare; the field-engineer underneath, with contractions, occasional muttered profanity, and first-name-as-actual-warmth rather than first-name-as-tool). The private register is **load-bearing**, when it appears, the chapter must earn it. Flag *both* directions: if a scene calls for the rare private register and Dorenne is still talking like she's in a Council briefing, that's a missed opportunity; if a scene casually slips into private register without earning it, that's a violation. Reference to her late husband **Marek** is private-register; should appear no more than once or twice across an entire book.
